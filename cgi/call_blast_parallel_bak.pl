...
system("echo 'qsub -t 1-$piece  -pe smp 1 -q one.q -sync yes  $cgi_dir/single_blast.pl  $faa_file_basename $database' >> $log_file");
system("qsub -t 1-$piece  -pe smp 1 -q one.q -sync yes  $cgi_dir/single_blast.pl  $faa_file_basename $database")==0 or system("echo $! >> $log_file");; 
