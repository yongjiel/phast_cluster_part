...
my $ptt_data = `cat ../$num.ptt`;
my %hash_ptt_data=();
foreach my $l (split "\n", $ptt_data){
	if ($l=~/(\d+)\.\.\d+\t\+\t\S+\t(\S+)/ or $l=~/\d+\.\.(\d+)\t\-\t\S+\t(\S+)/){
		$hash_ptt_data{$1}= $2;
	}
}
my $faa_data = '';
my $bac_database='';
my $virus_database=`cat $virus_db`;
my %hash_virus_database=$virus_database=~/gi\|(\d+)\|\w{3}\|\S+\|\s*(.*?)\n/gs;

my %hash_non_hit_blast_data=();
my %hash_bac_database=();
my %hash_faa_data=();
if ($NC eq 'NC_000000' or $flag eq '-s'){
	$bac_database=`cat $bac_db`;
	 # >gi|158333235|ref|YP_001514407.1| hypothetical protein AM1_0004 [Acaryochloris marina MBIC11017]
	foreach my $l(split "\n", $bac_database){
		if($l=~/(gi\|\d+\|\w{3}\|\S+\|)\s*(.*)/s){
			my $desc="$2 $1";$desc=~s/<*>*\d+\.\.<*>*\d+//;
			my ($k)= $l=~/gi\|(\d+)/;
	...
	if (-s $blast_output_file){
		...
		close $fh;
		#$blast_data = `cat  $blast_output_file`;
		foreach my $l (@array){
			if ($l=~/^gi\|(\d+)\|.*?gi\|(\d+)\|.*?\s+(\S+)\s+[\d\.]+$/){
				$hash_non_hit_blast_data{$1}="$2 $3" if (!defined $hash_non_hit_blast_data{$1});
			} 
		}
	}
}else{
	$faa_data = `grep '>' ../$num.faa`;
...
if (-e $scan_output_file){
	get_annotation($scan_output_file, \%hash_non_hit_blast_data, $faa_data, $num, \%hash_virus_database, \%hash_ptt_data, \%hash_bac_database, \%hash_faa_data);
	print "  Change $scan_output_file \n";
}
print "Finish annotation.pl\n\n";
exit(0);

# get annotation on specified file.
sub get_annotation{
	my ($scan_output_file ,$hash_non_hit_blast_data, $faa_data, $num, $hash_virus_database, $hash_ptt_data, $hash_bac_database, $hash_faa_hash)=@_;
