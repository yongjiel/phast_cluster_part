...
system("echo 'qsub -t 1-$piece  -pe smp 1 -q one.q -l h=\"$node_set\" -sync yes  $cgi_dir/single_blastplus.pl  $faa_file_basename $database' >> $log_file");
system("qsub -t 1-$piece  -pe smp 1 -q one.q -l h=\"$node_set\" -sync yes  $cgi_dir/single_blastplus.pl  $faa_file_basename $database")==0 or system("echo $! >> $log_file");; 
