...
public class CgviewFactoryTab implements CgviewConstants {
	...
	while((line = buf.readLine()) != null) {
		...
		if (lineItems.length >= 5) {
		    ...
		    if (hasOpacityColumn) {
			try {
			    if (!lineItems[opacityColumnIndex].equals("-")) {
			...
			try {
			    if (!lineItems[thicknessColumnIndex].equals("-")) {
		    ...
		    if (hasRadiusColumn) {
			try {
			    if (!lineItems[radiusColumnIndex].equals("-")) {
		    ...
		    if (hasLabelColumn) {
			try {
			    if (!lineItems[labelColumnIndex].equals("-")) {
			...
			try {
			    if (!lineItems[mouseoverColumnIndex].equals("-")) {
			...
			try {
			    if (!lineItems[hyperlinkColumnIndex].equals("-")) {
...
// 	}
	...
	cgview.setBackgroundColor((Color) MAP_ITEM_COLORS.get("background"));
	    ...
	    //if a title was provided, add it to the legend
	    if (!(title.matches("^\\s$"))) {	    
	...
	if (showWarning) {
	    ...
	    legendItem.setLabel("A maximum of " + (maxLabels * 2)+ " labels are shown per view.");
	    legendItem.setDrawSwatch(SWATCH_NO_SHOW);
	}
    }
}
