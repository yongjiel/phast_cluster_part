...
public class Cgview implements CgviewConstants {
    ...
    //some limits
    private int MAX_DNA_LENGTH = 10000000;
	...
	if (centerPoint != null) {
	    ...
	    at.setToTranslation((smallestDimension / 2) - centerPoint.getX(), (smallestDimension / 2) - centerPoint.getY());
	    gg.transform(at);

	    //create a rectangle for the background
	    backgroundRectangle = new Rectangle2D.Double(centerPoint.getX() - (smallestDimension / 2), centerPoint.getY() - (smallestDimension / 2), width, height);
	}
    }
	...
	AffineTransform at = new AffineTransform();
        at.setToTranslation((smallestDimension / 2), (smallestDimension / 2));
        gg.transform(at);

        backgroundRectangle = new Rectangle2D.Double(-(smallestDimension / 2), -(smallestDimension / 2), width, height);
...
//         //adjust bounds of labelBounds
//         i = labelBounds.iterator();
//         while (i.hasNext()) {
//             LabelBounds currentLabelBounds = (LabelBounds) i.next();
//             Rectangle2D bounds = currentLabelBounds.getBounds();
//             bounds.setRect(bounds.getX() + smallestDimension / 2 - centerPoint.getX(), bounds.getY() + smallestDimension / 2 - centerPoint.getY(), bounds.getWidth(), bounds.getHeight());
//             currentLabelBounds.setBounds(bounds);

//             //testing
//             //gg.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f));
//             //gg.setColor(Color.blue);
//             //gg.fill(currentLabelBounds.getBounds());
//             //gg.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1.0f));

//             //gg.fill(new Rectangle2D.Double(0.0d, 0.0d, 50.0d, 50.0d));
//             //end of testing
//         }
...
// 	tickSig[7] = 6;
        ...
        approxBasesPerTick = (double) (sequenceLength) / ((double) (desiredNumberOfTicks) * ((virtualBackboneRadius - (backboneRadius - this.getLastInnerFeatureRadius())) / 200.0d));
        ...
        while (!(finishedRanges)) {  //outer while loop
            ...
            while (j - 0.5d < endBase) {
                ...
                //if ((strand == REVERSE_STRAND)) {
                    ...
                    Rectangle2D tickBounds = new Rectangle2D.Double(tickBoundsX + smallestDimension / 2 - centerPoint.getX(), tickBoundsY + smallestDimension / 2 - centerPoint.getY(), tickLength + currentTickThickness + TICK_BOUNDS_PADDING, tickLength + currentTickThickness + TICK_BOUNDS_PADDING);
		    ...
		    //}
			    ...
			    untranslateCanvas();
			    layout.draw(gg, (float) (textPositionX + smallestDimension / 2 - centerPoint.getX()), (float) (textPositionY + smallestDimension / 2 - centerPoint.getY()) - layout.getDescent());
			    translateCanvas();
			    /////
        ...
        //draw a zero line if it will be visible
		...
		Rectangle2D tickBounds = new Rectangle2D.Double(tickBoundsX + smallestDimension / 2 - centerPoint.getX(), tickBoundsY + smallestDimension / 2 - centerPoint.getY(), tickLength + tickThickness + TICK_BOUNDS_PADDING, tickLength + tickThickness + TICK_BOUNDS_PADDING);
