...
public class CgviewFactoryPtt implements CgviewConstants {
    ...
    private int MAX_SEQUENCE_LENGTH = 10000000;
	...
	FEATURE_COLORS.put("N", new Color(62,217,157));
        ...
        LEGEND_ITEM_NAMES_SHORT.put("forward_gene", "Forward strand gene");
        LEGEND_ITEM_NAMES_SHORT.put("reverse_gene", "Reverse strand gene");
        ...
        DEFAULT_MAP_SIZES.put("labelFontSize", new Integer(12));
        ...
        DEFAULT_MAP_SIZES.put("backboneThickness", new Float(2.0f));
	...
	//pattern to find line with length value
        Pattern pLength = Pattern.compile("0\\.\\.(\\d+)\\s*$");
        Matcher m;
	...
	while((line = buf.readLine()) != null) {
	    ...
	    //look for map width value
	    if (line.startsWith("^")) {
		if (readDimension) {
		    try {
			mapWidth = Integer.parseInt(line.substring(1));
		    }
		    catch (Exception e) {
			throw new Exception("There is a problem with the width value on line " + lineCount + " in the data file.");
		    }
		}
	    }

	    //look for map height value
	    if (line.startsWith("$")) {
		if (readDimension) {
		    try {
			mapHeight = Integer.parseInt(line.substring(1));
		    }
		    catch (Exception e) {
			throw new Exception("There is a problem with the height value on line " + lineCount + " in the data file.");
		    }
		}
	    }
		...
		if (lineItems.length >= 5) {
		    ...
		    if (hasLengthColumn == false) {
			throw new Exception("A \"Length\" column has not been defined in the data file.");		
		    }
		    if (hasPIDColumn == false) {
			throw new Exception("A \"PID\" column has not been defined in the data file.");			
		    }
		    if (hasGeneColumn == false) {
			throw new Exception("A \"Gene\" column has not been defined in the data file.");			
		    }
		    if (hasSynonymColumn == false) {
			throw new Exception("A \"Synonym\" column has not been defined in the data file.");			
		    }
		    if (hasCodeColumn == false) {
			throw new Exception("A \"Code\" column has not been defined in the data file.");			
		    }
		    if (hasCOGColumn == false) {
			throw new Exception("A \"COG\" column has not been defined in the data file.");			
		    }
		    if (hasProductColumn == false) {
			throw new Exception("A \"Product\" column has not been defined in the data file.");			
		    }
		    ...
		    //product
		    try {
			product = lineItems[cogColumnIndex];
