...
public class CgviewIO implements CgviewConstants {
    ...
    private static void printUsage() {
        System.err.println("usage: java -jar cgview.jar [{-c,--centerBase} base to center on] [{-e,--excludeSVG} whether or not to exclude SVG from image series] [{-f,--formatOfOutput} output format] [{-h,--htmlFile} HTML file to create] [{-i,--inputFile} file to parse] [{-L,--legendSpace} space to reserve for external legend] [{-o,--outputFile} file to create] [{-p,--pathToImage} path to image in HTML file] [{-s,--seriesDirectory} directory to receive image series] [{-u,--useOverlib} whether or not to use JavaScript in HTML image maps] [{-z,--zoomAmount} zoom amount]");
        System.err.println("-c <integer> Specifies the base to center on when zooming. Optional. Default value is 1.");
        System.err.println("-e <boolean> Whether or not to exclude SVG output from image series. Optional. Default value is false.");
        System.err.println("-f <format> The format of the output: png, jpg, svg, or svgz. Optional. Default value is png.");
        System.err.println("-h <file>. HTML file to create. Optional. Default value is null.");
        System.err.println("-i <file> The XML file to parse. Required.");
        System.err.println("-L <integer> The width of an external legend to be included in HTML output. Optional. Default value is null.");
        System.err.println("-o <file> The image file to create. Required, unless -s used to specify that an image series is to be written to a directory.");
        System.err.println("-p <path> The path to the image file in the HTML file created using the -h option. Optional. Default is the the value specified using the -o option.");
        System.err.println("-s <directory> Directory to receive an image series. If this option is used, a series of zoomed views of the map is generated, and values for -c, -f, -h, -o, -p, -u, and -z are ignored. Optional. Default value is null.");
        System.err.println("-u <boolean> Whether or not to include calls to the overlib.js JavaScript library in HTML image maps for PNG and JPG images. Optional. Default value is false. When the -s option is used the -u option is automatically set to true.");
        System.err.println("-z <floating point number> The factor to zoom in by. Optional. Default value is 1.0.");
	...
	if (legendValue == null) {
        ...
        CgviewHTMLDocument htmlDocument;
        if (seriesValue != null) {
            htmlDocument = new CgviewHTMLDocument(true);
        } else {
            htmlDocument = new CgviewHTMLDocument(false);
        }

        htmlDocument.addHeader(title, Double.toString(cgview.getDesiredZoom()), Integer.toString(cgview.getDesiredZoomCenter()), imageFormat, useOverlibValue);
    ...
    public static void main(String args[]) {
        ...
        Boolean embedFontsValue = new Boolean(true);
        excludeSVGValue = new Boolean ((String)parser.getOptionValue(excludeSVG));
	...
	legendValue = (Integer) parser.getOptionValue(legend);
        ...
        useOverlibValue = new Boolean ((String)parser.getOptionValue(useOverlib));
	...
	if (useOverlibValue == null) {
	    useOverlibValue = new Boolean(false);
	}
	if (excludeSVGValue == null) {
	...
	if (inputValue.length() >= 3) {
	    ...
	    if (excludeSVGValue.booleanValue()) {
		if (!(fileMover.moveFile(INCLUDES_PATH, ZOOM_IN_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, ZOOM_OUT_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_FORWARD_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_BACK_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, INDEX_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, ZOOM_IN_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, ZOOM_OUT_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_FORWARD_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_BACK_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, INDEX_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, HELP_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, HELP_FILE_PNG, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, STYLE_FILE, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, OVERLIB_JAVASCRIPT, seriesValue + File.separator + INCLUDES_OUT_PATH))) {
		    System.err.println("Include files could not be copied to the " + seriesValue + File.separator + INCLUDES_OUT_PATH + " directory");
		    System.exit(1);
		}
	    }
            else {
		if (!(fileMover.moveFile(INCLUDES_PATH, ZOOM_IN_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, ZOOM_OUT_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_FORWARD_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_BACK_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, INDEX_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, ZOOM_IN_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, ZOOM_OUT_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_FORWARD_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, MOVE_BACK_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, INDEX_BUTTON_OFF, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, TO_SVG_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, TO_PNG_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, HELP_BUTTON, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, HELP_FILE, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, STYLE_FILE, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, SVG_JAVASCRIPT, seriesValue + File.separator + INCLUDES_OUT_PATH) && fileMover.moveFile(INCLUDES_PATH, OVERLIB_JAVASCRIPT, seriesValue + File.separator + INCLUDES_OUT_PATH))) {
		    System.err.println("Include files could not be copied to the " + seriesValue + File.separator + INCLUDES_OUT_PATH + " directory");
		    System.exit(1);
		}
            }

            //for now use the following for zoom values. Must start with 1, and must be in ascending order.
	    //Adjust zoom values based on the length of the sequence.
	    int[] zoomValues = {1, 6, 36, 216};
	    if (cgview.getSequenceLength() < 100000) {
		zoomValues = new int[2];
		zoomValues[0] = 1;
		zoomValues[1] = 6;
	    }
	    else if (cgview.getSequenceLength() < 1000000) {
		zoomValues = new int[3];		
		zoomValues[0] = 1;
		zoomValues[1] = 6;
		zoomValues[2] = 36;	    
	    }
