...
void GString::replace_data(int length) {
    ...
    if (length > 0) {
        ...
        GMALLOC(my_data, sizeof(Data) + length);
        my_data->ref_count = 1;
...
GString&  GString::tr(char *from, char* to) {
 ...
 if (to==NULL) { //deletion case 
   char* s = my_data->chars;
   char* p;
   char* dest = data->chars;
   do {
      if ((p=strpbrk(s,from))!=NULL) {
        memcpy(dest,s,p-s);
        dest+=p-s;
        s=p+1;
        }
   ...
   for (int i=0; i<length(); i++) {
    if ((p=strchr(from, my_data->chars[i]))!=NULL) 
         my_data->chars[i]=to[p-from];
    }
   }
...
GString&  GString::replace(const char *from, const char* to) {
 if (length() == 0 || from==NULL || strlen(from)==0)
        return *this;
 unsigned int l=strlen(from);
 ...
 if (tl!=l) { //reallocation
   if (tl>l) {  //possible enlargement
      ...
      if (tl==0) {//deletion
           while ((p=strstr(s,from))!=NULL) {
               //from found at position p
                memcpy(dest,s,p-s);
                dest+=p-s;
                s+=p-s+l; //s positioned in string after from
                }
          ...
          //not found any more, copy to end of string
          strcpy(dest, s);
          }
  ...
  else { //inplace editing: no need to reallocate
    while ((p=strstr(s,from))!=NULL) {
        memcpy(p,to,l);
        s+=p-s+l;
        }    
...
GString&  GString::cut(int index, int len) {
...
GString&  GString::paste(const GString& s, int index, int len) {
    // A negative index specifies an index from the right of the string.
    if (index < 0)
        index += length();
    make_unique(); //edit operation ahead
...
GString& GString::paste(const char *s, int index, int len) {
...
GString& GString::insert(const GString& s, int index) {
    make_unique(); //edit operation ahead
...
GString& GString::insert(const char *s, int index) {
    // A negative index specifies an index from the right of the string.
    make_unique(); //edit operation ahead
    if (index < 0)
        index += length();
...
int GString::index(const char *s, int start_index) const {
    ...
    const char* index = strstr(&chars()[start_index], s);
    if (!index)
        return -1;
    else
        return index - chars();
...
int GString::index(char c, int start_index) const {
    ...
    if (c == '\0')
        return -1;
    const char *index=(char *) ::memchr(&chars()[start_index], c,
                                         length()-start_index);
    if (!index)
        return -1;
    else
...
int GString::rindex(char c) const {   
    if (c == '\0' || length()==0)
        return -1;
    char* index= rstrchr((char*)chars(), c);
    if (index==NULL) return -1;
                else return index-chars();
}

int GString::rindex(char* str) const {
    if (str==NULL || *str == '\0' || length()==0)
        return -1;
    char* index= rstrfind((char*)chars(), str);
    if (index==NULL) return -1;
                else return index-chars();
}
...
bool GString::nextToken(GString& token) {
 ...
 if (fTokenizeMode==tkFullString) { //exact string as a delimiter
   ...
   int tlen=(token_end-chars())-fLastTokenStart+1;
   if (tlen==0) {
       GFREE(fTokenDelimiter);
       fLastTokenStart=0;
       return false;
